...
with open(r"G:\내 드라이브\안티그래비티\TEST\dashboard_app.py", "r", encoding="utf-8") as f:
    lines = f.readlines()

new_lines = []
for line in lines:
    if line.startswith("# =========================================="):
        # We will cut off right at the beginning of "5. 최상단 3대 KPI"
        if "5. 최상단 3대 KPI" in "".join(lines[lines.index(line):lines.index(line)+3]):
...
with open(r"G:\내 드라이브\안티그래비티\TEST\수수료분석\dashboard_app_new.py", "w", encoding="utf-8") as out:
    out.writelines(new_lines)
    out.write(part2)

import os
import shutil
shutil.copy(r"G:\내 드라이브\안티그래비티\TEST\수수료분석\dashboard_app_new.py", r"G:\내 드라이브\안티그래비티\TEST\dashboard_app.py")
