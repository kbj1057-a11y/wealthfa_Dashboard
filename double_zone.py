...
# Replace the entire section 9 (lines ~550 to ~630) with the new double container layout
...
# 9-0. Custom CSS for Double Container layout
...
<style>
    ...
    .zone-value-sub { color: var(--color-gold) !important; font-weight:900; font-size:3.0rem; margin:0; }
...
with zone_left:
    ...
    # [좌측 하단] 상/하 배치 혹은 좌/우 (여기서는 지시대로 나란히 배치)
    ...
    st.markdown("</div>", unsafe_allow_html=True) # End of left zone
...
with zone_right:
    ...
    with sub_r1:
        st.markdown(f"<div class='kpi-card-zone'><p class='zone-title'>③ 손해 익월</p><h2 class='zone-value-sub' style='font-size:2.4rem;'>{val_dmg_new:,.0f} <span class='unit-text'>원</span></h2></div>", unsafe_allow_html=True)
    ...
    with sub_r2:    
        st.markdown(f"<div class='kpi-card-zone'><p class='zone-title'>④ 손해 유지</p><h2 class='zone-value-sub' style='font-size:2.4rem;'>{val_dmg_ret:,.0f} <span class='unit-text'>원</span></h2></div>", unsafe_allow_html=True)
        st.markdown("<div class='table-toggle-container'><div class='table-toggle-btn'>상세 보기</div></div>", unsafe_allow_html=True)
    ...
    with sub_r3:
        st.markdown(f"<div class='kpi-card-zone'><p class='zone-title'>⑤ 손해 기타</p><h2 class='zone-value-sub' style='font-size:2.4rem;'>{val_dmg_etc:,.0f} <span class='unit-text'>원</span></h2></div>", unsafe_allow_html=True)
        st.markdown("<div class='table-toggle-container'><div class='table-toggle-btn'>상세 보기</div></div>", unsafe_allow_html=True)
    ...
    st.markdown("</div>", unsafe_allow_html=True) # End of right zone
...
if start_idx != -1 and end_idx != -1:
    ...
    print("Could not find section borders.")
